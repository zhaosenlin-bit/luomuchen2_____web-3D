...
const FALLBACK_PROJECTS = [
    {
        id: 'cosmic-explorer',
        title: '宇宙探索者',
        front: '/luomuchen2_____web-3D/media/apps/cosmic-home.webp',
        painted: '/luomuchen2_____web-3D/media/apps/cosmic-home.webp',
        url: '',
    ...
    },
    {
        id: 'cosmic-3d',
        title: '宇宙探索者 · 3D 视图',
        front: '/luomuchen2_____web-3D/media/apps/cosmic-3d.webp',
        painted: '/luomuchen2_____web-3D/media/apps/cosmic-3d.webp',
        url: '',
        description: '太阳系 3D 视图：鼠标拖动看行星，点击行星查看档案。我最满意的部分之一。',
        techStack: []
    },
    {
        id: 'cosmic-game',
        title: '宇宙探索者 · 5 阶段游戏',
        front: '/luomuchen2_____web-3D/media/apps/cosmic-game.webp',
        painted: '/luomuchen2_____web-3D/media/apps/cosmic-game.webp',
        url: '',
        description: '收集材料 → 建造火箭 → 飞船对接 → 躲避陨石 → 返回地球。5 个阶段，越玩越难。',
        techStack: []
    },
    {
        id: 'personal-site',
        title: '我的个人站（3D 卡通走廊）',
        front: '/luomuchen2_____web-3D/images/map.webp',
        painted: '/luomuchen2_____web-3D/images/map.webp',
        url: '',
        description: '把知识库搬进 3D 卡通走廊：6 扇门分别是关于我、学习领域、我的作品、概念乐园、每日记录、联系我。',
        techStack: []
    },
    {
        id: 'world-robot',
        ...
        url: '',
        description: '2025 年宜昌锦标赛 冠军 🏆。和队友一起搭建、调试、上台完成机器人比赛项目。',
        techStack: []
    },
    {
        ...
        url: '',
        description: '心理健康 × 科技方向的作品，具体项目内容待补充。',
        techStack: []
    },
    {
        id: 'my-knowledge-base',
        title: '我的 AI 学习知识库',
        front: '/luomuchen2_____web-3D/media/apps/knowledge-base.webp',
        painted: '/luomuchen2_____web-3D/media/apps/knowledge-base.webp',
        url: '',
        description: '我的第二大脑：每天记录、每项目一张卡、每概念一张卡、五大视图，让 AI 也懂我。',
        techStack: []
    },
]
...
const GalleryRoom = ({ projects, showRoom, onReady, isExiting, isWarmup, tutorialId = 'gallery_inspect' }) => {
    ...
    const backTextureRaw = useTexture(canHover ? '/luomuchen2_____web-3D/textures/gallery/tylkartki_painted.webp' : '/luomuchen2_____web-3D/textures/gallery/tylkartki.webp');
    const overlayTextureRaw = useTexture(canHover ? '/luomuchen2_____web-3D/textures/gallery/przyciskdotylukartki_painted.webp' : '/luomuchen2_____web-3D/textures/gallery/przyciskdotylukartki.webp');
    ...
    const allLogos = useMemo(() => {
        ...
        return names.map(name => {
            if (!canHover) return `/luomuchen2_____web-3D/textures/gallery/${name}.webp`;
            if (name === 'csslogo') return `/luomuchen2_____web-3D/textures/gallery/css3logo_painted.webp`;
            return `/luomuchen2_____web-3D/textures/gallery/${name}_painted.webp`;
        });
    }, [canHover]);
    ...
    // --- GEOMETRY & MATERIALS ---
    const floorTexture = useTexture('/luomuchen2_____web-3D/textures/gallery/floor.webp');
    const railingTexture = useTexture('/luomuchen2_____web-3D/textures/gallery/railing.webp');
    const housesTexture = useTexture('/luomuchen2_____web-3D/textures/gallery/domki.webp');
    const cityTexture = useTexture('/luomuchen2_____web-3D/textures/gallery/miastotlo.webp');
    const birdTexture = useTexture('/luomuchen2_____web-3D/textures/gallery/bird_gray.webp');
    const clothespinTexture = useTexture('/luomuchen2_____web-3D/textures/gallery/klamerka.webp');
    ...
    const materials = useMemo(() => {
        ...
        const thresholdMat = new THREE.MeshBasicMaterial({
            color: '#e0e0e0',
            map: (() => {
                const t = new THREE.TextureLoader().load('/luomuchen2_____web-3D/textures/corridor/texturadoprogow.webp');
    ...
    return (
        <group ref={groupRef}>
            {!isWarmup && (
                <PositionalAudio
                    ref={audioRef}
                    url="/luomuchen2_____web-3D/sounds/szummiasta.mp3"
...
{/* === TEKST NA PLECACH KARTKI (PROJECT DETAILS) === */}
                ...
                <PositionalAudio
                    ref={paperAudioRef}
                    url="/luomuchen2_____web-3D/sounds/papersound.mp3"
                    distanceModel="exponential"
