...
const DOOR_TEXTURES = {
    '关于我': '/luomuchen2_____web-3D/textures/corridor/doors/drzwiabout.webp',
    '学习领域': '/luomuchen2_____web-3D/textures/corridor/doors/drzwipractice.webp',
    '我的作品': '/luomuchen2_____web-3D/textures/corridor/doors/drzwiprojekty.webp',
    '概念乐园': '/luomuchen2_____web-3D/textures/corridor/doors/drzwisocial.webp',
    '每日记录': '/luomuchen2_____web-3D/textures/corridor/doors/drzwimoments.webp',
    '联系我': '/luomuchen2_____web-3D/textures/corridor/doors/drzwikontakt.webp',
};

// Painted (colored) variants for brush-stroke reveal on hover
const DOOR_PAINTED_TEXTURES = {
    '关于我': '/luomuchen2_____web-3D/textures/corridor/doors/drzwiabout_painted.webp',
    '学习领域': '/luomuchen2_____web-3D/textures/corridor/doors/drzwipractice_painted.webp',
    '我的作品': '/luomuchen2_____web-3D/textures/corridor/doors/drzwiprojekty_painted.webp',
    '概念乐园': '/luomuchen2_____web-3D/textures/corridor/doors/drzwisocial_painted.webp',
    '每日记录': '/luomuchen2_____web-3D/textures/corridor/doors/drzwimoments_painted.webp',
    '联系我': '/luomuchen2_____web-3D/textures/corridor/doors/drzwikontakt_painted.webp',
};
...
const DoorSection = ({
    ...
    // Load wall texture
    const originalWallTexture = useTexture('/luomuchen2_____web-3D/textures/corridor/wall_texture.webp');
    ...
    const doorTexturePath = DOOR_TEXTURES[label] || DOOR_TEXTURES['关于我'];
    ...
    const doorPaintedTexturePath = DOOR_PAINTED_TEXTURES[label] || DOOR_PAINTED_TEXTURES['关于我'];
    const doorPaintedTexture = useTexture(isTouch ? dummyTex : doorPaintedTexturePath);
    const frameTexture = useTexture('/luomuchen2_____web-3D/textures/corridor/doors/ramkasingledoors.webp');
    const handleTexture = useTexture('/luomuchen2_____web-3D/textures/corridor/doors/klamkadodrzwi.webp');
    const handlePaintedTexture = useTexture(isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/corridor/doors/klamkadodrzwi_painted.webp');
    const doorBackTexture = useTexture('/luomuchen2_____web-3D/textures/corridor/doors/backsingledoors.webp');
    const arrowTexture = useTexture('/luomuchen2_____web-3D/textures/corridor/strzalka.webp');

    // Baseboard texture for door sections (1582x94 px, aspect 16.83:1)
    const baseboardTexture = useTexture('/luomuchen2_____web-3D/textures/corridor/texturadoprogow.webp');
    ...
    // 门贴图 512x1024 宽高比 0.5,mesh 必须匹配避免纹理拉伸
    const doorWidth = doorHeight * 0.5;
    ...
    const signTextureUrl = '/luomuchen2_____web-3D/textures/corridor/pustatabliczka.webp';
    ...
    const labelFontSize = signHeight * 0.55;
        ...
        <group position={[pivotX, position[1], position[2]]}>
            ...
            <group ref={groupRef}>
                ...
                <group position={[wallOffsetX, -0.4, 0]}>
                    {/* === TEXTURED SIGN === */}
                    {/* Lifted higher above the door + pushed further in z to stay clear of door texture */}
                    <group position={[0, doorHeight / 2 + 0.75, 0.55]}>
                        {/* White paper backing so label reads clearly against the wood sign */}
                        <mesh position={[0, 0, -0.01]}>
                            <planeGeometry args={[signWidth * 0.94, signHeight * 0.62]} />
                            <meshBasicMaterial color="#fdfbf6" transparent opacity={0.92} />
                        </mesh>
                        <mesh>
...
{/* === DOOR FRAME (textured) === */}
                    ...
                    <group ref={doorRef} position={[doorPivotX, 0, 0.01]}>
                        ...
                        <mesh
                            ...
                            scale={[(side === 'right' && label !== '概念乐园') ? -1 : 1, 1, 1]}
                        >
                            <planeGeometry args={[doorWidth, doorHeight]} />
                            <meshBasicMaterial color="#e0e0e0"
                ...
                <PositionalAudio
                    ref={hoverAudioRef}
                    url="/luomuchen2_____web-3D/sounds/uchyleniedrzwi.mp3"
