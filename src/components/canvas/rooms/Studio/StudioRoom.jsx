...
const StudioRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    const monitorData = useMemo(() => {
        ...
        // Deterministic hash for stable order across re-renders (avoids flicker on window resize)
        const stableHash = (s) => { let h = 5381; for (let i = 0; i < s.length; i++) h = ((h << 5) + h) + s.charCodeAt(i); return h >>> 0; };
        let shuffledContent = [...activeContent].sort((a, b) => stableHash(a.id) - stableHash(b.id));
        ...
        if (shuffledContent.length > 0) {
            while (shuffledContent.length < 48) {
                shuffledContent = [...shuffledContent, ...[...activeContent].sort((a, b) => stableHash(a.id + 'x') - stableHash(b.id + 'x'))];
            }
        }
...
const MonitorBlock = memo(({ item, meshRef, isSelected, onMonitorClick, disabled, paintOnBeforeCompile, paintUniforms }) => {
    ...
    const frontTextureUrl = item.frontTexture || (
        isBlogMonitor ? '/luomuchen2_____web-3D/textures/studio/monitor_front.webp' :
            isTvMonitor ? '/luomuchen2_____web-3D/textures/studio/tv_front.webp' :
    ...
    const paintedFrontTextureUrl = isTouch ? dummyTex : (item.paintedFrontTexture || (
        isBlogMonitor ? '/luomuchen2_____web-3D/textures/studio/monitor_front_painted.webp' :
            isTvMonitor ? '/luomuchen2_____web-3D/textures/studio/tv_front_painted.webp' :
    ...
    const monitorBack = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/monitor_back.webp');
    const monitorTop = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/monitor_top.webp');
    const monitorBottom = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/monitor_bottom.webp');
    const monitorLeft = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/monitor_left.webp');
    const monitorRight = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/monitor_right.webp');
    const monitorBackPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/monitor_back_painted.webp');
    const monitorTopPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/monitor_top_painted.webp');
    const monitorBottomPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/monitor_bottom_painted.webp');
    const monitorLeftPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/monitor_left_painted.webp');
    const monitorRightPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/monitor_right_painted.webp');

    // Load TV textures (YouTube) - shell + painted
    const tvBack = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/tv_back.webp');
    const tvTop = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/tv_top.webp');
    const tvBottom = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/tv_bottom.webp');
    const tvSide = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/tv_side.webp');
    const tvBackPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/tv_back_painted.webp');
    const tvTopPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/tv_top_painted.webp');
    const tvBottomPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/tv_bottom_painted.webp');
    const tvSidePainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/tv_side_painted.webp');

    // Load Phone textures (TikTok) - shell + painted
    const phoneBack = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/phone_back.webp');
    const phoneSide = useLoader(TextureLoader, '/luomuchen2_____web-3D/textures/studio/phone_side.webp');
    const phoneBackPainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/phone_back_painted.webp');
    const phoneSidePainted = useLoader(TextureLoader, isTouch ? dummyTex : '/luomuchen2_____web-3D/textures/studio/phone_side_painted.webp');
