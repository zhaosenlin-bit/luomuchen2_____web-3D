...
const ContactRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    // Load Sea Texture
    const seaTexture = useTexture("/luomuchen2_____web-3D/textures/contact/faletopdown.webp");
    // Load Molo Texture
    const moloTexture = useTexture("/luomuchen2_____web-3D/textures/contact/molo.webp");
    // Load Latarnia Texture
    const latarniaTexture = useTexture("/luomuchen2_____web-3D/textures/contact/latarnia.webp");
    // Load Statek Texture
    const statekTexture = useTexture("/luomuchen2_____web-3D/textures/contact/statek.webp");
    ...
    return (
        <group ref={groupRef} position={[0, -0.7, -5]}>
            {!isWarmup && (
                <PositionalAudio
                    ref={audioRef}
                    url="/luomuchen2_____web-3D/sounds/szummorza.mp3"
            ...
            <group position={[0, -1, -8]}>
                {Array.from({ length: WAVE_LAYERS }).map((_, i) => (
                    ...
                    >
                        ...
                        {/* 🎯 靶子 SOCIAL BARRELS (骆沐辰的联系方式 · 隐私红线:不公开手机号/住址/学校) */}
            {/* 邮箱 */}
            <SocialBarrel
                position={isMobile ? [-1.4, 0.5, -10] : [-2.2, 0.5, -10]}
                rotation={[0, 0.2, 0]}
                texturePath="/luomuchen2_____web-3D/textures/contact/beczka.webp"
                label="邮箱"
                onClick={() => openOverlay({
                    layout: 'photo',
                    title: '邮箱 · 写信给我',
                    description: '这个站专属的邮箱正在准备中（待补充）。准备好后会放在这里。',
                    image: '/luomuchen2_____web-3D/media/qr/muchen-email-card.svg',
                    url: '',
                })}
            ...
            <SocialBarrel
                ...
                onClick={() => openOverlay({
                    layout: 'photo',
                    title: 'GitHub · 我的代码',
                    description: '我的 GitHub 账号待补充。宇宙探索者等项目的源码会放上去。',
                    image: '/luomuchen2_____web-3D/media/qr/muchen-github-card.svg',
                    url: '',
                })}
                paintOnBeforeCompile={onBeforeCompile}
                paintUniforms={uniformsData}
            ...
            <SocialBarrel
                ...
                onClick={() => openOverlay({
                    layout: 'photo',
                    title: '我的 AI 学习知识库',
                    description: '我的第二大脑：me / daily / projects / concepts / views 五大区。每天记录，让 AI 也懂我。',
                    image: '/luomuchen2_____web-3D/media/qr/muchen-kb-card.svg',
                    url: '',
                })}
                paintOnBeforeCompile={onBeforeCompile}
                paintUniforms={uniformsData}
