...
const SUBTITLES = {
    '关于我': '小创客 · Vibe Coding · 比赛选手',
    '学习领域': 'AI 编程 / 学校学习 / 概念卡',
    '我的作品': '宇宙探索者 · 个人站 · 机器人比赛',
    '概念乐园': '5 张概念卡 · 随时翻开',
    '每日记录': '每天 3 句话 · 我的成长足迹',
    '联系我': '邮箱 / GitHub / 留言 · 不公开隐私'
};
...
const RoomInterior = memo(({ label, showRoom, onReady, isExiting }) => {
    ...
    const floorTexSrc = useTexture('/luomuchen2_____web-3D/textures/corridor/kawalekpodlogi.webp');
    const wallTexSrc = useTexture('/luomuchen2_____web-3D/textures/corridor/wall_texture.webp');
    const ceilingTexSrc = useTexture('/luomuchen2_____web-3D/textures/corridor/ceiling_texture.webp');
    const bbTexSrc = useTexture('/luomuchen2_____web-3D/textures/corridor/texturadoprogow.webp');
    ...
    const isGallery = label === '我的作品';

    // Trigger onReady for generic rooms (which don't have their own component to do it)
    useEffect(() => {
        if (showRoom && !['关于我', '学习领域', '我的作品', '概念乐园', '每日记录', '联系我'].includes(label)) {
            onReady?.();
        }
